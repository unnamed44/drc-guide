...
const ThirdBossActions = {							// 3王攻击动作
	...
	112: {msg: '112 后扫(击退)'},
	301: {msg: '301 地刺(击飞)'},
	303: {msg: '303 →→→→右', sign_degrees1:  80, sign_distance1: 250, sign_degrees2:  260, sign_distance2: 250},
	306: {msg: '306 左←←←←', sign_degrees1: 280, sign_distance1: 250, sign_degrees2:  100, sign_distance2: 250},
	309: {msg: '309 注视!!'},
	315: {msg: '315 恐惧(吸血)'}
}
...
module.exports = function DarkReachCitadelGuide(d) {				// 定义变量
	...
	function sLoadTopo(event) {
		if (event.zone === mapID[0]) {
			insidemap = true;
			d.command.message('Добро пожаловать: ' + '泰内布利斯城堡 '.clr('56B4E9') + '[Нормал]'.clr('E69F00'));
			load();
		} else if (event.zone === mapID[1]) {
			insidemap = true;
			d.command.message('Добро пожаловать: ' + '泰内布利斯城堡 '.clr('56B4E9') + '[Хард]'.clr('00FFFF'));
			load();
		} else {
			unload();
		}
